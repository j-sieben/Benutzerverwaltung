--------------------------------------------------------
--  DDL for Package Body UI_ADMIN_ARF_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "UI_ADMIN_ARF_PKG" as
  ...
  begin
    ...
    select distinct p_arf_anw_id, column_value
      from table(utils.string_to_table(l_ref_anw_id_list))
  ...
  begin
    ...
    insert into bv_anwendung_referenz(arf_anw_id, arf_referenz_anw_id)
    select distinct p_arf_anw_id, column_value
      from table(utils.string_to_table(p_arf_anw_id));
